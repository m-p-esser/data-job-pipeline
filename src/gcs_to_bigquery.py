...
@task
def get_unprocessed_search_ids(load_dir: str, dataset_id: str, table_id: str) -> list:
   ...
   processed_search_ids = [b.name.split("_")[-1].split(".")[0] for b in processed_blobs if b.name != f"{load_dir}"]
   ...
   unprocessed_search_ids = list(set(processed_search_ids).difference(set(written_search_ids)))

   logger.info("INFO level log message")
   logger.info(f"There are {len(unprocessed_search_ids)} files which have not been written to Bigquery yet")
...
def write_search_metadata_to_bigquery_table(load_dir: str, file_name: str, dataset_id: str, table_id: str, schema: list, autodetect: bool):
   ...
   logger.info("INFO level log message")
   logger.info(f"Wrote data from file {file_name} to Bigquery table {dataset_id}.{table_id}")
...
def gcs_to_bigquery_flow(config: GCSToBigQueryConfig):
   ...
   unprocessed_search_metadata_search_ids = get_unprocessed_search_ids(load_dir=config.load_dir, dataset_id=config.dataset_id, table_id="search_metadata")
   ...
   for i in unprocessed_search_metadata_search_ids:
      ...
      write_search_metadata_to_bigquery_table(load_dir=config.load_dir, file_name=file_name, dataset_id=config.dataset_id, table_id="search_metadata", schema=schema, autodetect=False)

   # ----------------------------------------

   unprocessed_search_parameters_search_ids = get_unprocessed_search_ids(load_dir=config.load_dir, dataset_id=config.dataset_id, table_id="search_parameters")
   ...
   for i in unprocessed_search_parameters_search_ids:
      ...
      write_search_parameters_to_bigquery_table(load_dir=config.load_dir, file_name=file_name, dataset_id=config.dataset_id, table_id="search_parameters", schema=schema, autodetect=False)

   # ----------------------------------------

   unprocessed_job_results_search_ids = get_unprocessed_search_ids(load_dir=config.load_dir, dataset_id=config.dataset_id, table_id="job_results")
   ...
   for i in unprocessed_job_results_search_ids:
      ...
      write_job_results_to_bigquery_table(load_dir=config.load_dir, file_name=file_name, dataset_id=config.dataset_id, table_id="job_results", schema=None, autodetect=True)
